...
export function CrawlingView() {
  ...
  const [sinceDate, setSinceDate] = useState<string>(''); // State untuk tanggal mulai (YYYY-MM-DD)
  const [untilDate, setUntilDate] = useState<string>(''); // State untuk tanggal akhir (YYYY-MM-DD)
  ...
  const handleScrape = useCallback(async () => {
    ...
    console.log('DEBUG (Frontend): Since Date state value:', sinceDate);
    console.log('DEBUG (Frontend): Until Date state value:', untilDate);
    ...
    if (sinceDate && !dateRegex.test(sinceDate)) {
      setErrorMessage('Format tanggal mulai tidak valid. Gunakan YYYY-MM-DD.');
      setLoading(false);
      return;
    }
    if (untilDate && !dateRegex.test(untilDate)) {
      setErrorMessage('Format tanggal akhir tidak valid. Gunakan YYYY-MM-DD.');
      setLoading(false);
      return;
    }

    // Validasi logis tanggal (tanggal mulai tidak boleh lebih lambat dari tanggal akhir)
    if (sinceDate && untilDate && new Date(sinceDate) > new Date(untilDate)) {
      setErrorMessage('Tanggal mulai tidak boleh lebih lambat dari tanggal akhir.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/scraping/run', {
        ...
        params: {
          ...
          ...(sinceDate && { since_date_str: sinceDate }),
          ...(untilDate && { until_date_str: untilDate }),
        },
  ...
  }, [keyword, limit, sinceDate, untilDate, router]); // Tambahkan sinceDate & untilDate ke dependency array
    ...
    <DashboardContent>
      ...
      <Card sx={{ p: 3, mb: 4 }}>
        <Stack spacing={3}>
          ...
          <TextField
            ...
            helperText="Tanggal awal data yang ingin dicrawl. Format: Hari-Bulan-Tahun."
          />
          ...
          <TextField
            ...
            helperText="Tanggal akhir data yang ingin dicrawl. Format: Hari-Bulan-Tahun."
          />
      ...
      {scrapedData && scrapedData.length > 0 ? (
        ...
        !loading && (totalScraped === 0 || scrapedData.length === 0) && (
          <Alert severity="info" sx={{ mt: 2 }}>
            Jalankan proses crawling untuk memperoleh data.
          </Alert>
        )
      )}
    </DashboardContent>
